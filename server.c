...
int main() {
	...
	while (1) { //Terminal Loop
		...
		if(strcmp(net_buf, "listall") == 0)
		{
			...
			FILE *temporary_file = fopen("temp_file.txt", "w"); //creates temporary file on server and gives write permission

			while ((de = readdir(dir)) != NULL) {
				names[count++] = (char *)de->d_name;
				if(strcmp(de->d_name, "temp_file.txt") == 0 || strcmp(de->d_name, ".") == 0 || strcmp(de->d_name, "..") == 0 || !is_Regular(de->d_name))
					continue;
			...
			{ 
				...
				sendto(sockfd, net_buf, NET_BUF_SIZE, 
					sendrecvflag, (struct sockaddr*)&addr_con, addrlen); 				//keep sending
			...
			if(fp != NULL)
				fclose(fp);
			remove("temp_file.txt");//removes temporary file from server
			continue;	 
		}
